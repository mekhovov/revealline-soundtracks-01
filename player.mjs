...
async function play(row) {
  ...
  history.replaceState(null, '', `?track=${encodeURIComponent(track.id)}#recordings`);
  try {
    await audio.play();
  } catch {
    if (request === generation)
      status.textContent = 'Press Play in the audio controls to start. Your browser may require a gesture.';
  }
}
...
function renderTrack(track, index) {
  ...
  playButton.setAttribute('aria-label', `Play ${track.title} by ${track.artist}`);
  ...
  main.append(element('h2', '', track.title), element('p', 'artist', track.artist));
  const labels = [...track.tags];
  const duration = formatDuration(track.durationSeconds);
  if (duration) labels.push(duration);
  labels.push(track.gameCatalogueAdmission ? 'Game playlist' : 'Published audition');
  ...
  details.append(
    ...
    element('p', '', `File: ${track.fileName} · ${formatBytes(track.audio.bytes)}`),
...
async function loadCatalogue() {
  try {
    ...
    for (const track of catalogue.tracks)
      sourceCounts.set(track.archiveId, (sourceCounts.get(track.archiveId) ?? 0) + 1);
    for (const source of catalogue.sources) {
      const option = element('option', '', `${source.id === 'foundation-70' ? 'Foundation collection' : source.id} (${sourceCounts.get(source.id) ?? 0})`);
...
shuffle.addEventListener('change', () => { queue = []; });
playResults.addEventListener('click', () => { queue = []; next(); });
...
audio.addEventListener('pause', () => { pause.textContent = 'Resume'; });
audio.addEventListener('ended', next);
audio.addEventListener('error', () => {
  status.textContent = 'This recording could not load. Try Next or download its MP3.';
});
