"""Hosted-only entry point for the immutable four-recording Reckless 2 audition."""
import argparse
import hashlib
import json
import os
...
def checked_manifest(data):
    if len(data) > 64 * 1024 or hashlib.sha256(data).hexdigest() != MANIFEST_SHA256:
        raise ValueError('Immutable Reckless 2 source manifest changed; review a new batch')
...
def run(output, game_root):
    ...
    if (
        ...
        raise ValueError('Audio acquisition requires the distinct hosted Reckless 2 workflow')
    if output.exists():
...
if __name__ == '__main__':
    ...
    if args.check:
        ...
        print('Exact four-recording Reckless 2 source manifest verified; audio not acquired')
