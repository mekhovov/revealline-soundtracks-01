"""Hosted-only entry point for the immutable second twelve-recording audition slate."""
import argparse
import hashlib
import json
import os
...
MANIFEST_SHA256 = '54270c19d600c402004535283aa3044209fbc4e70e1d9926b04ec06a74067481'
...
def checked_manifest(data):
    if len(data) > 128 * 1024 or hashlib.sha256(data).hexdigest() != MANIFEST_SHA256:
        raise ValueError('Immutable second-directions source manifest changed; review a new batch')
...
def run(output, game_root):
    ...
    if (os.environ.get('GITHUB_ACTIONS') != 'true'
        ...
        raise ValueError('Audio acquisition requires the distinct hosted second-audition workflow')
    if output.exists():
    ...
    try:
        ...
        if output.is_dir():
            ...
            files = ('intake/second_directions_pins.py',
                     'intake/approved_directions_pins.py', 'intake/itch_source.py',
                     'intake/itch_audio.py', 'intake/prepare.py',
                     'intake/prepare_second_directions.py', WORKFLOW)
            binding = {
                'format': 'revealline-second-directions-intake-binding.v1',
...
if __name__ == '__main__':
    ...
    if args.check:
        ...
        print('Exact second twelve-recording source manifest verified; audio not acquired')
