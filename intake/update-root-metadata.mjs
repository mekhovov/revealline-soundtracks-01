...
export async function buildUpdatedRootManifest({
  ...
  if (
    ...
    throw new Error(
      "Refusing to rewrite an unexpected root deployment manifest.",
    );
  const objects = manifest.files.filter((entry) =>
    entry.path.startsWith("objects/"),
  );
  if (objects.length !== 70)
    throw new Error("Root audio pins changed or are incomplete.");
  const metadata = [];
...
if (
  ...
  console.log(
    "Updated root metadata pins without changing the 70 audio-object pins.",
  );
}
